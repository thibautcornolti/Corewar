...
** Last update Sat Apr  1 19:04:59 2017 Bastien
*/
...
int	st(t_inst *inst, t_ptr *ptr, t_map *map)
{
  ...
  my_memncpy (map->arena + (ptr->index_map + inst->arg[0].arg % IDX_MOD) % MEM_SIZE, &ptr->father->reg[inst->arg[1].arg - 1], 4);
  my_memncpy (map->color + (ptr->index_map + inst->arg[0].arg % IDX_MOD) % MEM_SIZE, &ptr->father->color, 4);
  return (0);
}
